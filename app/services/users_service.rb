...
class UsersService
  ...
  begin
    ...
    { success: true, message: "OTP request sent to queue", otp: @@otp, user_id: user.id }  # ✅ Include user_id in response
...
def self.reset_password(user_id, rp_params)
  ...
  if rp_params[:otp].to_i == @@otp && (Time.current - @@otp_generated_at < 1.minute)
    ...
    if user
      if user.update(password: rp_params[:new_password], password_confirmation: rp_params[:new_password]) # ✅ Hash password
        @@otp = nil  # ✅ Reset OTP after successful password change
        return { success: true, message: "Password reset successfully!" }
      else
        Rails.logger.error("❌ Password reset failed: #{user.errors.full_messages}") # ✅ Debug validation errors
        return { success: false, errors: user.errors.full_messages }  # ✅ Return validation errors
      end
    else
